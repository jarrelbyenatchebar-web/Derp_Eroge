
<<<<<<< HEAD
=======
define n = Character(None) #Narrator

define mc = Character(  #Rei/Player
    "[mc_name]",
    what_prefix="“",
    what_suffix="”"
)

define sd = Character(
    "Shinji",
    what_prefix="“",
    what_suffix="”"
)

define sz = Character(
    "Shizuku",
    what_prefix="“",
    what_suffix="”"
)

# =========================
# DEFAULT VARIABLES
# =========================
>>>>>>> refs/remotes/origin/main

default mc_name = "MC"



label scene_1_new_home:

    n "The rain stopped just as we arrived at the house."

    show rei normal:
        xalign 0.8
        yalign 1.0

    mc "{i}It felt strange.{/i}"
    mc "{i}Everything felt weird. Everything was new—houses, trees, cars.{/i}"
    mc "{i}Being stuck inside that shelter for so long left me clueless about the outside world.{/i}"

    hide rei normal

    # BG image WIP

    sd "Hey there, bud."
    sd "You okay? You look like you’ve never been outside before."
    sd "{i}*laughs*{/i}"

    mc "I… actually haven’t been outside in a long time."

    sd "Oh. Well—enjoy it as much as you like, kid."
    sd "It really is nice out here."

    #switch to adoption room bg WIP

    show shinji normal

    sd "{i}Poor kid. I hope he pulls through.{/i}"

    hide shinji normal

    mc "{i}Adopted.{/i}"
    mc "{i}Such a simple word for something that feels… strange.{/i}"
    mc "{i}New walls. New rules. New people.{/i}"
    mc "{i}I wonder how long it’ll take before they start feeling… real.{/i}"

    mc "{i}We arrived in this cozy neighborhood.{/i}"
    mc "{i}The rain stopped as my new adoptive \"Father\" parked his car.{/i}"
    mc "{i}When I stepped outside I caught a smell of the humid air left behind by the rain.{/i}"

    # home(front) bg WIP

    mc "{i}I went out the gate to take a better look around.{/i}"
    mc "{i}The neighborhood was quiet enough.{/i}"
    mc "{i}I found it quite peaceful compared to the barn they call shelter.{/i}"
    mc "{i}I take a look at the house and observed.{/i}"
    mc "{i}It was a two story house approximately for 3 people.{/i}"
    mc "{i}But why did they picked a house good for 3 people?{/i}"
    mc "{i}From what Shinji-san said, He only mentioned about having a step-daughter.{/i}"
    mc "{i}Did they already planned to adopt a child?{/i}"
    mc "{i}If so, how far did they already planned to adopt a child to get a house good for three?{/i}"
    mc "{i}Maybe there was another relative who lives in this house?{/i}"
    mc "{i}Or...{/i}"
    mc "{i}I might just be thinking about it too hard.{/i}"

    sd "[mc_name]-kun, lets get inside."

    # switch to living room bg WIP

    show shinji normal

    sd "Here we are."

    n "He unlocks the door and steps aside."

    sd "Make yourself at home, bud."
    sd "Don’t worry—you’re family now. No need to be nervous."

    mc "…Thank you. For taking me in."

    mc "{i}My voice feels like it’s breaking.{/i}"
    mc "{i}Or rather… shaking.{/i}"
    mc "{i}Say the right things.{/i}"
    mc "{i}Don’t mess this up.{/i}"

    sd "No need to thank me."

    n "He smiles—tired, but sincere."

    sd "Family isn’t always about blood."
    sd "Sometimes, it’s about timing."

    n "The house smelled faintly of tea and old wood."
    n "Warm. Quiet."
    n "Too quiet."

    sd "Ah—she should be upstairs."

    n "He raises his voice slightly."

    sd "Shizuku? We’re home."

    hide shinji normal

    n "Footsteps descended the stairs."

    sz "…You’re late."

    mc "{i}She stopped halfway down the stairs when she saw me.{/i}"
    mc "{i}Dark eyes—calm, quiet, unreadable.{/i}"
    mc "{i}Like she was empty.{/i}"

    sd "This is the boy I told you about."

    n "He gestures toward me."

    sd "His name is—"

    mc "I’m [mc_name]. Nice to meet you."

    mc "{i}Her gaze is sharp.{/i}"
    mc "{i}This isn’t curiosity.{/i}"
    mc "{i}It’s evaluation.{/i}"

    sz "…Welcome."

    n "Just one word."
    n "Yet it carried weight."

    sd "Well… she’s not very talkative at first."

    n "He chuckles awkwardly."

    sd "But she’s kind. In her own way."

    sz "…Dad."

    sd "Right. Sorry."

    mc "{i}So this is her.{/i}"
    mc "{i}The girl I’ll be sharing a roof with.{/i}"
    mc "{i}I can’t tell if she dislikes me…{/i}"
    mc "{i}Or if she’s just careful.{/i}"

    sz "…You can take the room at the end of the hall."

    mc "That’s more than enough. Thank you."
    mc "Oh—and, um—"

    mc "{i}I try to spark a conversation,{/i}"
    mc "{i}but she turns away instantly and walks back to her room.{/i}"

    return
